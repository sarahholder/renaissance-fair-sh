...
const editStaffMemberForm = (e) => {
  ...
  staffData.getStaffInfo(staffId)
    .then((resp) => {
      ...
      domString += `<input type="text" class="form-control" id="staffName" value="${staffPlaceholder.name}">`;
      domString += '</div>';
      domString += '<div class="form-group">';
      domString += '<label for="characterType">Edit the type of character</label>';
      domString += `<input type="text" class="form-control" id="characterType" value="${staffPlaceholder.characterType}">`;
      domString += '</div>';
      domString += '<div class="form-group">';
      domString += '<label for="staffImageUrl">Staff member photo</label>';
      domString += `<input type="text" class="form-control" id="staffImageUrl" value="${staffPlaceholder.imageUrl}">`;
      domString += '</div>';
      domString += '<div class="form-group">';
      domString += '<label for="characterName">Character name</label>';
      domString += `<input type="text" class="form-control" id="characterName" value="${staffPlaceholder.characterName}">`;
      domString += '</div>';
