...
const buildSouvenirsCards = (souvenir) => {
  ...
  firebase.auth().onAuthStateChanged((user) => {
    const cardsByUid = () => user.uid === souvenir.uid;
    if (cardsByUid !== user.uid) {
      console.log('could not print cards by uid', user.uid);
    } else {
      domString += `<div class="col-4 mb-2 souvenir-card" id="${souvenir.uid}">`;
